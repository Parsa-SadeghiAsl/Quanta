...
class TransactionViewSet(OwnerMixin, viewsets.ModelViewSet):
    ...
    def import_csv(self, request, *args, **kwargs):
        ...
        for i, row in enumerate(rows, 1):
            try:
                ...
                account, _ = Account.objects.get_or_create(name=account_name, user=user, defaults={'account_type': 'bank'})
                ...
                Transaction.objects.create(
                    ...
                    account=account,
                    category=category,
                    amount=Decimal(row_cleaned.get('amount', '0').replace('$', '').replace(',', '')),
                    date=datetime.strptime(row_cleaned.get('date'), '%Y-%m-%d').date(),
                    notes=row_cleaned.get('notes', '')
                )
                created_count += 1
            except Exception as e:
                error_detail = f"Row {i} skipped due to a data error: {e}. Row data: {row}"
    ...
    def export_csv(self, request, *args, **kwargs):
        ...
        writer.writerow(['date', 'account', 'category', 'category_type', 'amount', 'notes'])
        ...
        for tx in transactions:
            ...
            writer.writerow([tx.date, tx.account.name, category_name, category_type, tx.amount, tx.notes])
